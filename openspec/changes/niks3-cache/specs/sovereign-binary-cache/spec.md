...
Define the self-hosted niks3 binary cache service that provides a durable, S3-backed Nix cache with server-side signing, reference-tracking GC, and API-token-based push auth — without requiring a public homelab HTTP cache endpoint.
