...
The system SHALL compose Syncthing, Navidrome, slskd, Tagr, and optional SoulSync wiring from `modules/applications/music.nix` and SHALL define required collaboration groups for media operations.
...
Stateful media-stack services SHALL support backup coverage for their mutable service state. Coverage for media payloads under `/srv/media` SHALL be controlled by host backup policy.
...
- **AND** media library and inbox payloads under `/srv/media` follow the host's declared backup scope and exclusions
