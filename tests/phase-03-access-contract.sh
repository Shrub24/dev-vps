...
USERS_FILE="modules/core/users.nix"
...
rg --fixed-strings --quiet 'path:.#nixosConfigurations.${HOST}.config' "$JUSTFILE"
...
rg --fixed-strings --quiet 'derive-host-age host=target_host port="22" key_alias="host_generic_age" sops_file=".sops.yaml" update="false":' "$JUSTFILE"
rg --fixed-strings --quiet 'ssh-keyscan -p "{{port}}" -t ed25519 "{{host}}"' "$JUSTFILE"
rg --fixed-strings --quiet 'derive-host-age-from-key ssh_pubkey key_alias="host_generic_age" sops_file=".sops.yaml" update="false":' "$JUSTFILE"
