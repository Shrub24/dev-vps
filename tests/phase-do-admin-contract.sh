...
nix eval --no-write-lock-file --raw path:.#nixosConfigurations.do-admin-1.pkgs.system | rg --fixed-strings --quiet 'x86_64-linux'
...
nix eval --no-write-lock-file --raw path:.#nixosConfigurations.do-admin-1.config.services.admin.termix.dataDir | rg --fixed-strings --quiet '/srv/data/termix'
nix eval --no-write-lock-file --apply 'enabled: enabled == true' path:.#nixosConfigurations.do-admin-1.config.services.pocket-id.enable | rg --fixed-strings --quiet 'true'
nix eval --no-write-lock-file --apply 'v: v == "https://id.shrublab.xyz"' path:.#nixosConfigurations.do-admin-1.config.services.admin.pocket-id.appUrl | rg --fixed-strings --quiet 'true'
nix eval --no-write-lock-file --apply 'v: v == "https://id.shrublab.xyz"' path:.#nixosConfigurations.do-admin-1.config.services.pocket-id.settings.APP_URL | rg --fixed-strings --quiet 'true'
nix eval --no-write-lock-file --apply 'v: v == "/run/secrets/pocket-id.encryption_key"' path:.#nixosConfigurations.do-admin-1.config.services.pocket-id.settings.ENCRYPTION_KEY_FILE | rg --fixed-strings --quiet 'true'
nix eval --no-write-lock-file --apply 'v: v == true' path:.#nixosConfigurations.do-admin-1.config.services.admin.termix.oidc.enabled | rg --fixed-strings --quiet 'true'
nix eval --no-write-lock-file --apply 'v: v != null' path:.#nixosConfigurations.do-admin-1.config.services.admin.termix.oidc.environmentFile | rg --fixed-strings --quiet 'true'
nix eval --no-write-lock-file --apply 'v: v != {}' path:.#nixosConfigurations.do-admin-1.config.services.gatus.settings.security.oidc | rg --fixed-strings --quiet 'true'
nix eval --no-write-lock-file --apply 'v: v != null' path:.#nixosConfigurations.do-admin-1.config.services.gatus.environmentFile | rg --fixed-strings --quiet 'true'
nix eval --no-write-lock-file --apply 'v: v == "true"' path:.#nixosConfigurations.do-admin-1.config.services.beszel.hub.environment.DISABLE_PASSWORD_AUTH | rg --fixed-strings --quiet 'true'
...
rg --fixed-strings --quiet '"issuer-url" = cfg.pocketIdBaseUrl;' 'modules/applications/admin.nix'
rg --fixed-strings --quiet '"redirect-url" = "https://gatus.shrublab.xyz/authorization-code/callback";' 'modules/applications/admin.nix'
rg --fixed-strings --quiet 'DISABLE_PASSWORD_AUTH = "true";' 'modules/applications/admin.nix'
rg --fixed-strings --quiet 'USER_CREATION = "true";' 'modules/applications/admin.nix'
...
rg --fixed-strings --quiet 'pocket_id:' 'hosts/do-admin-1/secrets.template.yaml'
rg --fixed-strings --quiet 'cloudflare_access:' 'hosts/do-admin-1/secrets.template.yaml'
rg --fixed-strings --quiet 'apps:' 'hosts/do-admin-1/secrets.template.yaml'
