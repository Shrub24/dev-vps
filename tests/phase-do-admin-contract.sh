...
nix eval --no-write-lock-file --apply 'v: v == "https://id.shrublab.xyz"' path:.#nixosConfigurations.do-admin-1.config.services.shrublab-pocket-id.appUrl | rg --fixed-strings --quiet 'true'
...
nix eval --no-write-lock-file --apply 'v: v == true' path:.#nixosConfigurations.do-admin-1.config.services.termix.oidc.enabled | rg --fixed-strings --quiet 'true'
nix eval --no-write-lock-file --apply 'v: v != null' path:.#nixosConfigurations.do-admin-1.config.services.termix.oidc.environmentFile | rg --fixed-strings --quiet 'true'
