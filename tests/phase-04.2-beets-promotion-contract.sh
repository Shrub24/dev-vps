...
rg --fixed-strings --quiet 'UNTAGGED_ROOT="/srv/media/quarantine/untagged"' "$BEETS_RUNNER"
...
rg --fixed-strings --quiet 'd /srv/media/quarantine 2775 root music-ingest - -' "$BEETS_MODULE"
rg --fixed-strings --quiet 'd /srv/media/quarantine/untagged 2775 root music-ingest - -' "$BEETS_MODULE"
rg --fixed-strings --quiet 'd /srv/media/quarantine/approved 2775 root music-ingest - -' "$BEETS_MODULE"
rg --fixed-strings --quiet 'a+ /srv/media/quarantine/untagged - - - - group:media:r-x' "$BEETS_MODULE"
rg --fixed-strings --quiet 'a+ /srv/media/quarantine/untagged - - - - default:group:media:r-X' "$BEETS_MODULE"
rg --fixed-strings --quiet 'a+ /srv/media/quarantine/approved - - - - group:media:r-x' "$BEETS_MODULE"
rg --fixed-strings --quiet 'a+ /srv/media/quarantine/approved - - - - default:group:media:r-X' "$BEETS_MODULE"
...
rg --fixed-strings --quiet 'skipping demotion because destination already exists' "$BEETS_RUNNER"
rg --fixed-strings --quiet 'return 1' "$BEETS_RUNNER"
rg --fixed-strings --quiet 'if demote_and_record_unresolved "$file"; then' "$BEETS_RUNNER"
...
if rg --fixed-strings --quiet '/srv/media/library/untagged' "$BEETS_MODULE" ||
	rg --fixed-strings --quiet '/srv/media/library/untagged' "$BEETS_RUNNER" ||
	rg --fixed-strings --quiet '/srv/media/library/untagged' "$BEETS_CONFIG"; then
	echo 'untagged path must live under /srv/media/quarantine/untagged'
	exit 1
fi

if rg --fixed-strings --quiet '/srv/media/library/tagged' "$BEETS_MODULE" ||
	rg --fixed-strings --quiet '/srv/media/library/tagged' "$BEETS_RUNNER" ||
	rg --fixed-strings --quiet '/srv/media/library/tagged' "$BEETS_CONFIG"; then
	echo 'legacy tagged path must not exist; library root is /srv/media/library'
	exit 1
fi
...
if rg --fixed-strings --quiet 'MusicFolder = "/srv/media/library"' "$NAVIDROME_FILE"; then
	echo 'navidrome must not drift to /srv/media/library-only visibility'
	exit 1
fi

rg --fixed-strings --quiet 'MusicFolder = "/srv/media"' "$NAVIDROME_FILE"
rg --fixed-strings --quiet 'systemd.services.beets-quarantine-promote-run' "$BEETS_MODULE"
rg --fixed-strings --quiet 'ExecStart = "${beetsQuarantineApprovedRunner}/bin/beets-quarantine-approved-runner /srv/media/quarantine/approved";' "$BEETS_MODULE"
rg --fixed-strings --quiet 'systemd.paths.beets-quarantine-promote-watch' "$BEETS_MODULE"
rg --fixed-strings --quiet 'pathConfig.PathModified = "/srv/media/quarantine/approved";' "$BEETS_MODULE"
rg --fixed-strings --quiet 'systemd.timers.beets-quarantine-promote-backstop' "$BEETS_MODULE"
rg --fixed-strings --quiet 'OnUnitActiveSec = "20m"' "$BEETS_MODULE"
rg --fixed-strings --quiet '/srv/media/quarantine/approved | /srv/media/quarantine/approved/*' "$BEETS_RUNNER"
