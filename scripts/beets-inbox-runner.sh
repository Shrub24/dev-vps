...
if ((IMPORT_AS_IS == 1)); then
  cp "${BEETS_APPROVED_CONFIG_SOURCE:-${BEETS_CONFIG_SOURCE:?missing BEETS_CONFIG_SOURCE}}" "$BEETS_DATA_DIR/config.yaml"
  chmod 0640 "$BEETS_DATA_DIR/config.yaml"
fi
...
IMPORT_CMD=(beet -c "$BEETS_DATA_DIR/config.yaml" import -q -C -l "$IMPORT_LOG_FILE" "$CANONICAL_TARGET")
...
approved_retry_imported=0
if ((IMPORT_AS_IS == 1)) && ((${#LEFTOVER_AUDIO[@]} > 0)); then
  for file in "${LEFTOVER_AUDIO[@]}"; do
    if [[ ! -f "$file" ]]; then
      continue
    fi
    if beet -c "$BEETS_DATA_DIR/config.yaml" import -q -C -l "$IMPORT_LOG_FILE" "$file"; then
      ((approved_retry_imported += 1))
    fi
  done
...
echo "beets-inbox-runner: summary target=$CANONICAL_TARGET candidates=${#CANDIDATES[@]} imported_estimate=$imported_estimate leftovers=${#LEFTOVER_AUDIO[@]} demoted=$demoted_count approved_retry_imported=$approved_retry_imported import_log=$IMPORT_LOG_FILE runner_log=$RUNNER_LOG_FILE"
